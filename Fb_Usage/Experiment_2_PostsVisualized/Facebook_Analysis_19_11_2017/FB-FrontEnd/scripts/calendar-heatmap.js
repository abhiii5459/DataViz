...
function calendarHeatmap() {
  ...
  //var colorRange = ['#D8E6E7', '#218380'];
    var colorRange = ['#d4c4cd', '#f00000'];

  var tooltipEnabled = true;
  var tooltipUnit = 'post';
  ...
  function chart() {
    ...
    d3.select(chart.selector()).selectAll('svg.calendar-heatmap').remove(); // remove the existing chart, if it exists
      var startTime = new Date("2008-03-30T10:02:03+05:30"); // added

//  var dateRange = d3.time.days(yearAgo, now); // generates an array of date objects within the specified range
    var dateRange = d3.time.days(startTime, now); // generates an array of date objects within the specified range
    var monthRange = d3.time.months(moment(startTime).startOf('month').toDate(), now); // it ignores the first month if the 1st date is after the start of the month
